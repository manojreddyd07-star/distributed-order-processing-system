...
CREATE INDEX IF NOT EXISTS idx_fulfillments_active ON fulfillment(fulfillment_status, created_at DESC)
WHERE fulfillment_status IN ('PENDING', 'IN_PROGRESS', 'PACKAGING');

-- Index for fulfillment by order
CREATE INDEX IF NOT EXISTS idx_fulfillments_order_status_updated ON fulfillment(order_id, fulfillment_status, updated_at DESC);

-- Composite index for fulfillment tracking
CREATE INDEX IF NOT EXISTS idx_fulfillments_tracking ON fulfillment(tracking_number, fulfillment_status)
WHERE tracking_number IS NOT NULL;

-- Index for fulfillment audit trail
CREATE INDEX IF NOT EXISTS idx_fulfillment_audit_created ON fulfillment_audit_log(fulfillment_id, created_at DESC, action);
...
WHERE retry_status = 'PENDING' AND next_retry_time IS NOT NULL;

-- Add comments for documentation
COMMENT ON INDEX idx_fulfillments_active IS 'Partial index for active fulfillment monitoring';
COMMENT ON INDEX idx_fulfillments_order_status_updated IS 'Optimizes order fulfillment status queries';
