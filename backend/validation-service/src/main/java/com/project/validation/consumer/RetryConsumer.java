...
public class RetryConsumer {
    ...
    /**
     * Consumes RetryEvent from retry-orders topic
     * Only processes events meant for this service
     */
    @KafkaListener(
    ...
    public void consumeRetryEvent(
        ...
        logger.info("Received RetryEvent from topic: {}, partition: {}, offset: {}", 
                   topic, partition, offset);
        logger.info("Retry event details: {}", event);
        
        try {
            // Only process retry events for this service
            ...
            // Check if retry should be processed now
            if (event.getNextRetryTime() != null && 
                event.getNextRetryTime().isAfter(LocalDateTime.now())) {
            ...
            // Handle retry failure (will reschedule or mark as exhausted)
